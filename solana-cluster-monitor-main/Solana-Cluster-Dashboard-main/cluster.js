...
//     firstFundedAt: now,
//     lastSeenAt: now,
//     balanceHistory: [{ t: now, bal: lamports }], // Initialize with funding amount
...
//     cluster.cachedSpendRateLamportsPerSec = 0;
//     cluster.cachedTimeRemainingSec = 0;
...
//       if (timeElapsedSec >= 3) { // Reduced to 3 seconds
//         if (spentLamports >= 0) { // Allow zero spend
//           cluster.cachedSpendRateLamportsPerSec = spentLamports / timeElapsedSec;
//           if (totalRemaining > 0 && cluster.cachedSpendRateLamportsPerSec > 0) {
//             cluster.cachedTimeRemainingSec = Math.floor(totalRemaining / cluster.cachedSpendRateLamportsPerSec);
//           }
//         }
//       }
...
//     const remainingSOL = lamportsToSOL(totalRemaining);
//     const spendRateSOL = cluster.cachedSpendRateLamportsPerSec * 60;
...
//       ? lamportsToSOL(cluster.cachedRemainingLamports) 
//       : 0;
//     const spendRatePerMin = cluster.cachedSpendRateLamportsPerSec * 60;
//     const timeRemaining = spendRatePerMin > 0 && remainingSOL > 0 
//       ? Math.floor((remainingSOL / spendRatePerMin) * 60)
//       : 0;
...
async function updateBalances(connection) {
  ...
  for (const cluster of activeClusters) {
    ...
    cluster.cachedTimeRemainingSec = totalRemaining > 0 ? 3600 : 0; // Default to 1 hour if funds remain
        ...
        if (spentLamports >= 0) {
          ...
          if (totalRemaining > 0) {
            cluster.cachedTimeRemainingSec = cluster.cachedSpendRateLamportsPerSec > 0
              ...
              : 3600; // Default to 1 hour if no spending
          }
        }
      }
    } else {
